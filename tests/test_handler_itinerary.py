...
from src.data_pipeline.handlers.handler_itinerary import build_itinerary_sample, process_itinerary_data


def test_build_itinerary_sample_removes_preface_platform_and_stale_detail_lines() -> None:
    rng = random.Random(0)
    record = {
        "destination": "东京",
        "days": "3",
...
def test_build_itinerary_sample_rejects_overlong_content() -> None:
    rng = random.Random(0)
    ...
    assert build_itinerary_sample(record, rng) is None


def test_build_itinerary_sample_rejects_time_dense_schedule() -> None:
    rng = random.Random(0)
    record = {
        "destination": "阿尔山",
        "days": "2",
...
def test_process_itinerary_data_filters_risky_sample() -> None:
    good_record = {
        "destination": "香港",
        "days": "3",
        "itinerary_content": (
    ...
    temp_dir = Path(".tmp_itinerary_test")
    if temp_dir.exists():
        shutil.rmtree(temp_dir)
    temp_dir.mkdir(parents=True, exist_ok=True)
    try:
        input_path = temp_dir / "itinerary.jsonl"
        output_path = temp_dir / "sft_itinerary.json"
        ...
        dataset = process_itinerary_data(str(input_path), str(output_path), seed=0)
